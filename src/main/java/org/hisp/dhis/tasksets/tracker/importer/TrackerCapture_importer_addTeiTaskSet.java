    ...
    extends DhisAbstractTask
{
    ...
    public void execute()
    {
        ...
        if ( response.extractImportedEnrollments().size() == 0) {
        ...
        response = new AddTrackerEventsTask(1, entitiesCache, Lists.newArrayList(event) , user.getUserCredentials() ).executeAndGetResponse();

        if (response.extractImportedEvents().size() == 0)
        {
        ...
        dataValueSet.forEach( dv -> {
            taskSet.addTask(
                new AddTrackerDataValueTask( 1, eventId, new DataValueMapperImpl().from(
