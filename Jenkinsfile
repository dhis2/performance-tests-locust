...
//        copyArtifactPermission("$JOB_BASE_NAME")
    ...
    parameters {
        ...
        string(name: 'INSTANCE_URL', defaultValue: 'https://Kpk.Pakdhis2.org', description: 'Which instance to target?')
        string(name: 'TIME', defaultValue: '30m', description: 'How much time to run the tests for?')
        string(name: 'USERS', defaultValue: '10', description: 'How much users?')
        string(name: 'RATE', defaultValue: '1', description: 'At what rate to add users?')
        password(name: 'ADMIN_PASSWORD', defaultValue: '', description: 'Admin user password')
...
//        INSTANCE_URL = "https://${env.INSTANCE_HOST}/${env.INSTANCE_NAME}"
        COMPOSE_ARGS = "NO_WEB=true TIME=${params.TIME} HATCH_RATE=${params.RATE} USERS=${params.USERS} TARGET=${params.INSTANCE_URL} MASTER_HOST=master ADMIN_PASSWORD=${params.ADMIN_PASSWORD}"
